...
def main():
    ...
    parser.add_argument('--hybrid', action='store_true', help='Run in hybrid mode with Tkinter UI for results but OpenCV window for camera')
    ...
    # Normal UI mode setup
    def on_video_selected(video_path):
        run_detection_loop(video_path, ui)
        
    def on_camera_selected():
        # If hybrid mode was requested, use that
        if args.hybrid:
            ui.window.withdraw()  # Hide main window temporarily
            run_hybrid_camera(ui)  # Run in hybrid mode
            return
